...
public final class Money implements MonetaryAmount, Comparable<MonetaryAmount>, Serializable{
     ...
     */
    public static Money sum(MonetaryAmount a, MonetaryAmount b) {
    	  MoneyUtils.checkAmountParameter(Objects.requireNonNull(a), Objects.requireNonNull(b).getCurrency());
    	  Money ma = convertToMoney(a);
    	  Money mb = convertToMoney(b);
    	  return ma.add(mb);
    }

    /**
     * Returns the smaller of two {MonetaryAmount int} values.
     * If the arguments have the same value, the result is that same value.
     * @param   a   an argument.
     * @param   b   another argument.
     * @return  the smaller of {@code a} and {@code b}.
     */
    public static Money min(MonetaryAmount a, MonetaryAmount b) {
  	  MoneyUtils.checkAmountParameter(Objects.requireNonNull(a), Objects.requireNonNull(b).getCurrency());
  	  Money ma = convertToMoney(a);
  	  Money mb = convertToMoney(b);
  	  return ma.isLessThan(mb)? ma: mb;
  }
    /**
     * Returns the greater of two {@code MonetaryAmount} values.
     ...
     */
    public static Money max(MonetaryAmount a, MonetaryAmount b) {
    	  MoneyUtils.checkAmountParameter(Objects.requireNonNull(a), Objects.requireNonNull(b).getCurrency());
    	  Money ma = convertToMoney(a);
    	  Money mb = convertToMoney(b);
    	  return ma.isGreaterThan(mb)? ma: mb;
    }

	private static Money convertToMoney(MonetaryAmount a) {
		if (Money.class.isInstance(a)) {
			return Money.class.cast(a);
		}
		return Money.of(a.getNumber(), a.getCurrency());
	}
