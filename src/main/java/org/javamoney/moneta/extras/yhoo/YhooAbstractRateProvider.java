 ...
 */
abstract class YhooAbstractRateProvider extends AbstractRateProvider implements
        LoaderListener {

	private static final Logger LOG = Logger.getLogger(YhooAbstractRateProvider.class.getName());

    private static final String BASE_CURRENCY_CODE = "USD";

    private final String DIGIT_FRACTION_KEY = "yhoo.digit.fraction";
    ...
    YhooAbstractRateProvider(ProviderContext context) {
        super(context);
    ...
    public void newDataLoaded(final String resourceId, final InputStream is) {
        ...
        try {
        	final YhooRateReadingHandler parser = 
        			new YhooRateReadingHandler(rates, getContext());
            parser.parse(is);
