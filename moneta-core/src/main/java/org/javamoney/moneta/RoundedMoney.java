...
public final class RoundedMoney implements MonetaryAmount, Comparable<MonetaryAmount>, Serializable {
	...
	public static final MonetaryContext DEFAULT_MONETARY_CONTEXT = MonetaryContextBuilder.of(RoundedMoney.class)
			.set("MonetaryRounding", Monetary.getDefaultRounding()).build();
	 ...
	 * @param number   the amount, not null. @deprecated, will be private, use of()
	 */
	 ...
	 * @param number
	 * @param currency
	 * @param mathContext @deprecated, will be private, use of()
	 */
	public RoundedMoney(Number number, CurrencyUnit currency, MathContext mathContext) {
		...
		this.monetaryContext = DEFAULT_MONETARY_CONTEXT.toBuilder().set("MonetaryRounding", rounding).set(mathContext)
				.build();
	 ...
	 * @param currency
	 * @param context
	 * @param rounding @deprecated, will be private, use of()
	 */
	public RoundedMoney(Number number, CurrencyUnit currency, MonetaryContext context, MonetaryOperator rounding) {
		...
		this.rounding = RoundedMoneyMonetaryOperatorFactory.INSTANCE.getDefaultMonetaryOperator(rounding, context,
	 ...
	 * For example, {@code ofMinor(USD, 1234)} creates the instance
	 * {@code USD 12.34}.
	 * 
	 * @param currency    the currency, not null
	...
	/**
	 * Obtains an instance of {@code RoundedMoney} from an amount in minor units.
	 * For example, {@code ofMinor(USD, 1234, 2)} creates the instance
	 * {@code USD 12.34}.
	 * 
	 * @param currency      the currency, not null
	...
	public RoundedMoney add(MonetaryAmount amount) {
		...
		return new RoundedMoney(number.add(amount.getNumber().numberValue(BigDecimal.class)), currency, rounding)
				.with(rounding);
	}
	...
	public RoundedMoney divide(Number divisor) {
		...
		BigDecimal dec = number.divide(bd,
				Optional.ofNullable(monetaryContext.get(RoundingMode.class)).orElse(RoundingMode.HALF_EVEN));
	...
	public RoundedMoney[] divideAndRemainder(Number divisor) {
		...
		BigDecimal[] dec = number.divideAndRemainder(MoneyUtils.getBigDecimal(divisor),
				Optional.ofNullable(monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64));
	...
	public RoundedMoney divideToIntegralValue(Number divisor) {
		BigDecimal dec = number.divideToIntegralValue(MoneyUtils.getBigDecimal(divisor),
				Optional.ofNullable(monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64));
	...
	public RoundedMoney subtract(MonetaryAmount amount) {
		...
		return new RoundedMoney(
				number.subtract(amount.getNumber().numberValue(BigDecimal.class),
						Optional.ofNullable(monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64)),
				currency, rounding);
	}
	...
	public RoundedMoney remainder(Number divisor) {
		return new RoundedMoney(
				number.remainder(MoneyUtils.getBigDecimal(divisor),
						Optional.ofNullable(monetaryContext.get(MathContext.class)).orElse(MathContext.DECIMAL64)),
				currency, rounding);
	}
	 ...
	 * @return the new amount with the same numeric value and {@link MathContext},
	 *         but the new {@link CurrencyUnit}.
	 */
	...
	/**
	 * @param amt amount
	 * @return A new RoundedMoney based on the given amount
	 */
	...
	private static MonetaryAmountFormat defaultFormat() {
		String useDefault = MonetaryConfig.getConfig().getOrDefault("org.javamoney.moneta.useJDKdefaultFormat",
				"false");
		try {
	...
	private boolean isOne(Number number) {
		...
		try {
			...
			// The only way to end up here is that longValueExact throws an
			// ArithmeticException,
